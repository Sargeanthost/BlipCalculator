...
/**
 * Calculates basic offsets for a fall or jump.
 */
public class TierCalculator {
    ...
    /**
     * Calculates and prints basic offsets for a fall or jump.
     */
    public void calculateAndOutputTier() {
        TierHelper tierHelper = new TierHelper();
        if (!selected) {
            System.out.println("Offsets: ");
            tierHelper.offsetList(startingHeight, false).stream().filter(f -> f > 0)
                .map(height -> tierHelper.entranceGenerator(height, false)).forEach(System.out::println);
            System.out.println();
        } else {
            System.out.println("Offsets: ");
